...
public class ChestConversionItem extends ChestModifierItem
{
	...
	private void upgradeCursedChest(World world, BlockPos pos, BlockState state)
	{
		...
		world.setBlockState(pos, Registry.BLOCK.get(to).getDefaultState().with(Properties.HORIZONTAL_FACING, state.get(Properties.HORIZONTAL_FACING)).with(Properties.WATERLOGGED, state.get(Properties.WATERLOGGED)).with(
                CursedChestBlock.TYPE, state.get(CursedChestBlock.TYPE)));
	...
	private void upgradeChest(World world, BlockPos pos, BlockState state)
	{
		...
		world.removeBlockEntity(pos);
		world.setBlockState(pos, Registry.BLOCK.get(to).getDefaultState().with(Properties.HORIZONTAL_FACING, state.get(Properties.HORIZONTAL_FACING)).with(Properties.WATERLOGGED, state.get(Properties.WATERLOGGED)).with(CursedChestBlock.TYPE, CursedChestType
                .valueOf(state.get(Properties.CHEST_TYPE))));
