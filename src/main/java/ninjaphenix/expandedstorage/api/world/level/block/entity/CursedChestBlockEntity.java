package ninjaphenix.expandedstorage.api.world.level.block.entity;
    ...
    private static int countViewers(World world, CursedChestBlockEntity instance, int x, int y, int z)
    {
        ...
        while (true)
        {
            ...
            do
            {
                ...
                } while (!(player.container instanceof ScrollableMenu));
                inventory = ((ScrollableMenu) player.container).getInventory();
            } while (inventory != instance && (!(inventory instanceof SidedCompoundContainer) || !((SidedCompoundContainer) inventory).isPart(instance)));
            viewers++;
        }
    }
